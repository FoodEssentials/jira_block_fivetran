    ...
    sql:
       SELECT
              ...
              _product_name.name AS product_name,
               ...
               version.fix_version_list,
               issue_link.related_issues_list
        ...
        LEFT JOIN (
          SELECT id, STRING_AGG(name, ', ') as component_list
          FROM jira.component
          GROUP BY 1
        ) as component
        ...
        LEFT JOIN (
          SELECT id, STRING_AGG(name, ', ') as fix_version_list
          FROM jira.version
          GROUP BY 1
        ) as version
        ...
        LEFT JOIN (
          SELECT issue_id, STRING_AGG(CAST(related_issue_id AS STRING), ', ') as related_issues_list
          FROM jira.issue_link
          GROUP BY 1
        ) as issue_link
